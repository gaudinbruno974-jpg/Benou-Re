// Import d'un répertoire (Membres, Visiteurs ou Dignitaires) depuis un
// classeur .xlsx : l'écran ne lit et n'écrit que la feuille correspondant à
// [category], même si le classeur choisi contient aussi les deux autres
// feuilles (c'est le cas du classeur produit par « Exporter »). Déroulé :
// sélection du fichier → aperçu des lignes détectées (avec choix ligne par
// ligne pour chaque doublon) → confirmation récapitulative → écriture
// réelle. Voir directory_xlsx_service.dart pour la détection de doublon et
// la résolution de chaque ligne.
...
class DirectoryImportScreen extends StatefulWidget {
  ...
  const DirectoryImportScreen({super.key, required this.category});
...
class _DirectoryImportScreenState extends State<DirectoryImportScreen> {
  ...
  Future<void> _pickFile() async {
    ...
    try {
      ...
      final List<Object> rows = switch (widget.category) {
        DirectoryCategory.members => parseMemberSheet(bytes, state.members),
        DirectoryCategory.visitors => parseVisitorSheet(bytes, state.visitors),
        DirectoryCategory.dignitaries =>
          parseDignitarySheet(bytes, state.dignitaries),
      };
      setState(() {
        _fileName = file.name;
        _rows = rows;
  ...
  Future<void> _confirmAndWrite() async {
    final toWrite = _rows.where((r) => _rowAction(r) != ImportAction.skip).toList();
    final creations = toWrite.where((r) => _rowAction(r) == ImportAction.create).length;
    final updates = toWrite.where((r) => _rowAction(r) == ImportAction.update).length;
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text(
          ...
          'action écrit directement dans la base de la Loge.',
    ...
    return ListView(
      children: [
        Text(
          ...
          style: const TextStyle(color: BrColors.muted, fontSize: 13, height: 1.4),
        ),
        ...
        OutlinedButton.icon(
          ...
          onPressed: () => exportDirectoryTemplate(context),
        ),
      ],
    );
  }
  ...
  Widget _buildPreview() {
    final creations = _rows.where((r) => _rowAction(r) == ImportAction.create).length;
    final updates = _rows.where((r) => _rowAction(r) == ImportAction.update).length;
    ...
    return Column(
      ...
      children: [
        ...
        if (_rows.isEmpty)
          ...
          Expanded(
            child: ListView(
              children: [for (final row in _rows) _ImportRowCard(row: row, onChanged: () => setState(() {}))],
            ),
          ),
        const SizedBox(height: 12),
        SizedBox(
        ...
        child: Row(
          children: [
            ...
            DropdownButton<ImportAction>(
              ...
              onChanged: (v) {
                _setRowAction(row, v ?? (hasExisting ? ImportAction.update : ImportAction.create));
                onChanged();
              },
            ),
          ],
